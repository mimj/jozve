...
export default {
    ...
    mutations: {
        ...
        updatePagesForTopic(state, pages) {
            let topic = state.allTopics.find((topic) => topic.id === state.currentTopicId);

            // state.allTopics = state.allTopics.map(topic => {
            //     if (topic.id === state.currentTopicId) {
            //         return Object.assign({}, topic, pages)
            //     }
            //     return topic
            // });

            Object.assign(topic, {'pages': pages});
            state.allTopics = [
                ...state.allTopics.filter((topic) => topic.id !== state.currentTopicId),
                topic
            ];
        ...
        setCurrentPage(state, pageId, parentPage_id) {
            ...
            if (pageId == null) {
                ...
                state.currentPage = currentTopic.pages.find(page => page.id === Number(pageId));
            }
    ...
    actions: {
        ...
        getPages({commit, state}, parentPageId) {
            ...
            return axios.get('/' + state.currentTopicId + "/pages", {params: {parentPageId}})
                .then((res) => {
                    debugger
                    console.log('res', res)
                    commit('updatePagesForTopic', res.data);
                    return res.data
                })
        ...
        addPageTitleToTopic({commit, state}, pageTitle) {
            let topic = state.allTopics.find((topic) => topic.id === state.currentTopicId);
            ...
            let data = {
                ...
                prevPagePosition: pageTitle.prevPagePosition,
                page_content: null
            };
